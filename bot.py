...
def ask_gemini_with_media(prompt: str, images: list) -> str:
    """images = list of bytes objects"""
    try:
        parts = []
        for img_bytes in images:
            parts.append(types.Part(
                inline_data=types.Blob(data=bytes(img_bytes), mime_type="image/jpeg")
            ))
...
async def water_tip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    response = ask_gemini(
        f"Сейчас {hour}:00. Короткий совет по водному балансу с учётом времени суток. "
        f"Норма воды, как распределить, 1-2 лайфхака."
    )
...
async def menu_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["products_text"] = []
    context.user_data["products_images"] = []
    await update.message.reply_text(
...
async def menu_show_add_or_compose(update: Update, context: ContextTypes.DEFAULT_TYPE, summary: str):
    texts = context.user_data.get("products_text", [])
    images_count = len(context.user_data.get("products_images", []))
    info = []
    if texts:
        info.append(f"Текст: {'; '.join(texts)}")
    if images_count:
        info.append(f"Фото: {images_count} шт.")
    if summary:
        info.append(f"Распознано: {summary}")
...
async def menu_got_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get("menu_state") not in ("waiting_products", "waiting_add_or_compose"):
        return
    ...
    recognized = ask_gemini_with_media(
        "Перечисли все продукты питания на фото. Только список через запятую.",
        [image_bytes],
    )

    if recognized is None:
        recognized = "[фото добавлено, продукты будут распознаны при составлении меню]"

    context.user_data.setdefault("products_images", []).append(image_bytes)
    context.user_data.setdefault("products_text", [])
    await menu_show_add_or_compose(update, context, recognized)


async def build_menu_prompt(meal_type: str, context: ContextTypes.DEFAULT_TYPE) -> tuple:
    """Возвращает (prompt_text, images_list)"""
    texts = context.user_data.get("products_text", [])
    images = context.user_data.get("products_images", [])

    products_str = ""
    if texts:
        products_str += "Продукты (текст): " + "; ".join(texts) + "\n"
    if images:
        products_str += f"Дополнительно прилагаются {len(images)} фото с продуктами.\n"

    prompt = f"{products_str}\n{MEAL_PROMPTS[meal_type]}"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ожидаем продукты (текст)
    if menu_state == "waiting_products":
        context.user_data.setdefault("products_text", []).append(user_text)
        context.user_data.setdefault("products_images", [])
        await menu_show_add_or_compose(update, context, "")
        return

    # Добавить ещё или составить меню
    ...
    if menu_state == "waiting_meal_type":
        ...
        prompt, images = await build_menu_prompt(user_text, context)

        if images:
            response = ask_gemini_with_media(prompt, images)
            if response is None:
