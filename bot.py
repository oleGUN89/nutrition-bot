...
7. Ответы короткие и конкретные - никаких вступлений и приветствий
8. Сразу переходи к сути"""
...
def recognize_photo(image_bytes: bytes) -> str | None:
        ...
        parts = [
            ...
            types.Part.from_text(
                text="Перечисли все продукты питания, которые видишь на фото. "
                     ...
                     "Если продуктов нет — напиши 'продукты не распознаны'."
            ),
        ]
...
async def menu_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["recognized_products"] = []
    await update.message.reply_text(
...
async def show_add_or_compose(update: Update, context: ContextTypes.DEFAULT_TYPE, new_item: str):
    products = context.user_data.get("recognized_products", [])
    count = len(products)
    context.user_data["menu_state"] = "waiting_add_or_compose"
    await update.message.reply_text(
        f"Добавлено ({count} поз.): {new_item}\n\nЧто дальше?",
...
async def menu_got_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    recognized = recognize_photo(image_bytes)

    if recognized and "не распознаны" not in recognized.lower():
        context.user_data.setdefault("recognized_products", []).append(recognized)
        await show_add_or_compose(update, context, recognized)
    else:
        await update.message.reply_text(
            "Не удалось распознать продукты на фото. Опиши их текстом.",
            reply_markup=ReplyKeyboardMarkup([["Отмена"]], resize_keyboard=True),
        )
        context.user_data["menu_state"] = "waiting_products"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ожидаем продукты текстом
    if menu_state == "waiting_products":
        context.user_data.setdefault("recognized_products", []).append(user_text)
        await show_add_or_compose(update, context, user_text)
        return
    ...
    if menu_state == "waiting_meal_type":
        ...
        products = context.user_data.get("recognized_products", [])
        products_str = "\n".join(f"- {p}" for p in products) if products else "продукты не указаны"
        prompt = f"Имеющиеся продукты:\n{products_str}\n\n{MEAL_PROMPTS[user_text]}"
