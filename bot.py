...
Форматирование (Telegram HTML — строго):
- Названия блюд и заголовки разделов: <b>Текст</b>
- Списки ингредиентов и шаги: каждый пункт с новой строки, начиная с символа •
- Между блоками (завтрак/обед/ужин и т.д.) — пустая строка
- Калорийность пиши в скобках: (~NNN ккал)
- Никаких символов * и ** — только HTML-теги <b> и <i>
- Никаких других HTML-тегов кроме <b> и <i>"""
...
MEAL_PROMPTS = {
    "На весь день": (
        "Составь план питания на день. Используй структуру:\n"
        "<b>Завтрак</b> (~400 ккал) — название блюда, затем ингредиенты с граммовкой через •, приготовление 2-3 шага через •\n\n"
        "<b>Обед</b> (~550 ккал) — аналогично\n\n"
        "<b>Ужин</b> (~380 ккал) — аналогично\n\n"
        "<b>Перекус</b> (~180 ккал) — аналогично\n\n"
        "<b>Докупить</b> — 5-7 позиций через •"
    ),
    "Завтрак": (
        "Составь 2 варианта завтрака (~400 ккал каждый).\n"
        "Для каждого: <b>Название</b>, ингредиенты с граммовкой через •, "
        "приготовление 2-3 шага через •, калорийность."
    ),
    "Обед": (
    ...
    "Перекус": (
        ...
        "Для каждого: <b>Название</b>, ингредиенты с граммовкой через •, калорийность."
    ),
}
...
async def snack_ideas(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    response = ask_gemini(
        ...
        "Для каждого: <b>название</b>, калорийность, почему подходит."
    )
    await update.message.reply_text(
